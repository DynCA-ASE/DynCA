...
PbOCCSATSolver::PbOCCSATSolver(string inst_dir, int reference_num_vars)
{
	version_str = "1.2.3";
	inst = inst_dir;
...
{
	if ((int) flip_priority.size() != num_vars)
	{
...
bool PbOCCSATSolver::build_instance(int reference_num_vars)
{
	...
	for (c = 0; c < num_clauses;)
	{
		...
		while (cur_lit != 0)
		{
			...
			{
				lit_redundent = false;
				for (i = 0; i < (int) temp_clause.size(); i++)
				{
...
int PbOCCSATSolver::pick_var_pbocc()
{
	...
	{
		if (Random::next_double() < para_prob_pac)
		{
	...
	{
		if (Random::next_double() < para_prob_first_div)
		{
...
{
	int v;
	if ((int) init_soln.size() != num_vars)
	{
		cout << "c" << endl;
...
{
	if ((int) usr_defined_var_priority.size() != num_vars + 1)
	{
		cout << "c" << endl;
...
void PbOCCSATSolver::build_neighbor_relation()
{
	...
	for (v = 1; v <= num_vars; ++v)
	{
		...
		count = 0;
		for (i = 0; i < (int) temp_neighbor.size(); i++)
		{
...
{
	int c = Random::bound(num_clauses);
	return clause_lit[c][Random::bound(clause_lit_count[c])].var_num;
}

int PbOCCSATSolver::select_unsat_clause_random_heuristic()
{
	return unsat_stack[Random::bound(unsat_stack_fill_pointer)];
}
...
int PbOCCSATSolver::select_unsat_clause_usr_defined_priority_heuristic()
{
	...
	int best_c = unsat_stack[Random::bound(unsat_stack_fill_pointer)];
...
int PbOCCSATSolver::select_unsat_clause_weight_distribution_heuristic()
{
	...
	{
		random_value = Random::bound(total_unsat_clause_weight) + 1;
	}
	else
	{
		random_prob = Random::next_double();
	...
	if (c == -1)
		c = unsat_stack[Random::bound(unsat_stack_fill_pointer)];
	return c;
}
...
int PbOCCSATSolver::select_var_from_clause_usr_defined_priority_heuristic(int clause_num)
{
	...
	best_v = clause_lit[c][Random::bound(clause_lit_count[c])].var_num;
	for (i = 0; i < clause_lit_count[c]; i++)
	{
		v = clause_lit[c][i].var_num;
...
int PbOCCSATSolver::select_var_from_clause_novelty_heuristic(int clause_num)
{
	...
	{
		if (score[best_v] == 0 || Random::next_double() < para_prob_novelty)
...
int PbOCCSATSolver::select_var_by_random_break_tie_in_set(int *the_set, int the_set_length)
{
	...
	if (the_set_length == 1)
		return the_set[0];
	return the_set[Random::bound(the_set_length)];
}
...
int PbOCCSATSolver::select_var_by_usr_defined_var_priority_break_tie_in_set(int *the_set, int the_set_length)
{
	...
	best_v = the_set[Random::bound(the_set_length)];
...
{
	if (Random::next_double() < para_smooth_probability_paws && large_weight_clauses_count > large_clause_weight_count_threshold)
...
string PbOCCSATSolver::get_last_level_name(string file_path)
{
	...
	if (pos < 0 || pos >= (long long) file_path.size())
		return file_path;
	else
