...
{
    pbo_solver_ = new pboccsat::PbOCCSATSolver(cnf_file_path_, num_var_);
}
...
vector<int> CDCLCASampler::GetWeightedSampleInitSolution()
{
    ...
    for (int v = 0; v < num_var_; v++)
    {
        if (Random::next_double() < var_positive_sample_weight_[v])
        {
...
void CDCLCASampler::SetUpperLimit(long long limit)
{
    upperlimit = limit;
    use_upperlimit = true;
}
